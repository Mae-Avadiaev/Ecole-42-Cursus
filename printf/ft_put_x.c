...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/12/17 20:59:48 by mclown            #+#    #+#             */
/*   Updated: 2020/12/18 10:40:22 by mclown           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

void	part_one_x(struct s_specifier *sp, int len)
{
	if (sp->width > 0)
	{
		if (sp->prec >= 0)
...
void	ft_put_x(unsigned int n, struct s_specifier *sp)
{
	...
	part_one_x(&(*sp), len);
	(n == 0 && sp->prec == 0) ? 0 :
		(sp->total += mod_ft_putnbr_hex_low_fd(n, 1));
