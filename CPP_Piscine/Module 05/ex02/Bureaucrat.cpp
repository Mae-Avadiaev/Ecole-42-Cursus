...
void Bureaucrat::signForm(int gradeToSign, std::string formName) const {
	if (_grade > gradeToSign)
		std::cout	<< _name << " cannot sign " << formName
					 << " because " << GTLTS << std::endl;
	else
...
void Bureaucrat::executeForm(const Form &form) {
	if (form.isSigned()) {
		if (_grade <= form.getGradeToExecute()) {
			...
			throw GradeTooLowException(GTLTEF,
									   __FILE__,
									   __LINE__,
									   __FUNCTION__,
									   "");
